...
export default function App({ items }) {
    return (
        <div className="flex space-x-1">
            {
                items.map((el, i) => <div key={Math.random()} className="flex items-center">
                    <div
                    key={Math.random()} 
                    className="flex bg-gray-200 px-2 justify-between rounded items-center w-22 py-1">
                    <span className="px-1">{bread}</span>
