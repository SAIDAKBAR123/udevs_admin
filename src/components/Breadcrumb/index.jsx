        ...
        <div className="flex pb-3">
            {
                items.map((el, i) => <div key={Math.random()} className="flex items-center text-sm">
                    <div
                        ...
                        <span className="pr-1">{ items.length !== i + 1 ? <Bread className="fill-current text-gray-300"/> : <Bread className="fill-current text-blue-400"/>}</span>
                        <span className={`${items.length !== i+1 ? 'cursor-pointer' : ''} px-1`}>{el.title}</span>
                        { items.length !== i + 1 ? <div className="mr-3 ml-1">{arrow}</div> : '' }
                    </div> 
                </div>
                )
            }
